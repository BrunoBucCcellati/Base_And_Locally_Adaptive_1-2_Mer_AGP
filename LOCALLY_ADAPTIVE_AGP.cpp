...
pair <double, double> Locally_Adaptive_1_2_Mer_Algoritm(double a, double b, double epsilon, double r, unsigned short N, unsigned short m, double epsilon_granichnoe, double eta_0, unsigned short global_local_iterations)
{
    ...
    if (N == 1)
    {
        ...
        while (abs(pred_i_sled_shag.second - pred_i_sled_shag.first) > epsilon)
        {
            ...
            interval->Add(promejutochnaya_tochka, N, eta_0, global_local_iterations, K, a, b, epsilon_granichnoe);
            K++;
            chislo_iteraciy++;
        }
        return min_xy;
    ...
    else
    {
        ...
        while (max(abs(pred_i_sled_shag.second - pred_i_sled_shag.first), max(abs(pred_i_sled_shag_PI_Na_Dva.second - pred_i_sled_shag_PI_Na_Dva.first), abs(pred_i_sled_shag_Minus_PI_Na_Dva.second - pred_i_sled_shag_Minus_PI_Na_Dva.first))) > epsilon)
        {
            ...
            interval_PI_Na_Dva->Add(promejutochnaya_tochka_PI_Na_Dva, N, eta_0, global_local_iterations, schetchick, a, b, epsilon_granichnoe);
            interval_Minus_PI_Na_Dva->Add(promejutochnaya_tochka_Minus_PI_Na_Dva, N, eta_0, global_local_iterations, schetchick, a, b, epsilon_granichnoe);
            schetchick++;
            chislo_iteraciy++;
        }
        return min_xy;
...
int main()
{
    //pair <double, double> Extr = Locally_Adaptive_1_2_Mer_Algoritm(0.0, 1.0, 0.000005, 4.0, 2, 11, pow(10.0, -6.0), pow(10.0, -6.0), 10);
    pair <double, double> Extr = Locally_Adaptive_1_2_Mer_Algoritm(0.0, 10.0, 0.00005, 2.0, 1, -1, pow(10.0, -6.0), pow(10.0, -6.0), 10);
